...
export async function buildWeb({ outDir = join(repoRoot, 'dist') } = {}) {
  ...
  await Promise.all([mkdir(assetsDir, { recursive: true }), mkdir(evidenceDir, { recursive: true })]);
  await Promise.all([
    copyFile(join(webDir, 'index.html'), join(outDir, 'index.html')),
  ...
  return Object.freeze({ capsuleSha256, events: capsule.flightRecord.length, output: outDir });
}
