...
test('public capsule drives the complete safe and accessible flight recorder', async () => {
  ...
  try {
    ...
    assert.match(hub, /The flight recorder for/); assert.match(hub, /Connect your local signer/);
    assert.match(hub, /pnpm connector/); assert.match(hub, /Import pairing file/);
    assert.match(hub, /not your DID or private key/); assert.match(hub, /one operator, two distinct DIDs/i);
    ...
    assert.match(identity, /How to create a Technocore DID/i); assert.match(identity, /Rescan local identities/i); assert.match(identity, /Automatic rotation/);
    assert.doesNotMatch(identity, /Create my identity/i);
    assert.match(technocore, /Verify signed message/i); assert.match(technocore, /NO WRITE CONTROL/); assert.match(technocore, /not a complete Technocore history/i); assert.match(technocore, /wallet ownership/);
    assert.match(hubApp, /MULTIPLE_IDENTITIES_FOUND/); assert.doesNotMatch(hubApp, /identity\/create\//); assert.match(hubApp, /identity\/primary/);
    assert.match(records, /Verified historical reference/); assert.match(connectorClient, /sessionStorage/);
    ...
    assert.match(html, /“verified” means a local receipt and exact public-value match—not payment, settlement, or value transfer/);
    assert.match(hubApp, /EXISTING_BLACKBOX_PROFILE' \? 'Existing Local Profile'/);
