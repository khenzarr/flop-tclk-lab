...
// phase3a104.test.mjs is a seventh exception: cross-process durability cannot be
// proven inside one process, so it launches short Node children that touch only
// the dependency-free attempt-budget module under a temporary fixture root.
const spawnApi=/\bnode:child_process\b/;
// W2 custody is another reviewed signer-only subprocess boundary; it never invokes a shell.
const spawnAllowed=new Set([join('blackbox','typecheck.mjs'),join('lab','compat-matrix.mjs'),join('lab','candidate-probe.mjs'),join('blackbox','airlock','detached-bridge.mjs'),join('blackbox','workloads','validation-publication','custody-bridge.mjs'),join('blackbox','tests','phase3a7.test.mjs'),join('blackbox','tests','phase3a9.test.mjs'),join('blackbox','tests','phase3a103.test.mjs'),join('blackbox','tests','phase3a104.test.mjs')]);
...
const networkAllowed=new Set([join('web','dev-server.mjs'),join('web','connector-client.js'),join('web','tests','hub-flight-record.test.mjs'),join('web','tests','workload-w1-flow.test.mjs'),join('blackbox','hub','connector.mjs'),join('blackbox','workloads','transcript-validation','import.js'),join('blackbox','tests','connector-production-origin.test.mjs'),join('blackbox','tests','deal-hub-v1.test.mjs'),join('blackbox','tests','identity-hub.test.mjs'),join('blackbox','tests','workload-w2.test.mjs'),join('blackbox','tests','workload-w2-terminal-retry.test.mjs')]);
