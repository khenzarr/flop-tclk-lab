...
const networkAllowed=new Set([join('web','dev-server.mjs'),join('web','connector-client.js'),join('web','tests','hub-flight-record.test.mjs'),join('web','tests','workload-w1-flow.test.mjs'),join('blackbox','hub','connector.mjs'),join('blackbox','workloads','transcript-validation','import.js'),join('blackbox','tests','connector-production-origin.test.mjs'),join('blackbox','tests','deal-hub-v1.test.mjs'),join('blackbox','tests','identity-hub.test.mjs')]);
