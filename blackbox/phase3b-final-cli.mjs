...
import { FINAL_ORDER, signPreflight, runRealSign, submitPreflight, runRealSubmit, runRealObserve,
  railPreflight, railWrite, railObserve, finalizeFinal, finalW1RecoveryPreflight, runFinalW1RecoverySubmit } from './phase3b-s2.mjs';

if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const [command, flag, id, extra] = process.argv.slice(2); const needsId = command !== 'finalize';
  if (!['sign', 'submit', 'submit-recovery', 'observe', 'rail-preflight', 'rail-write', 'rail-observe', 'finalize'].includes(command)
    || (needsId && (flag !== '--operation' || !FINAL_ORDER.includes(id) || (command === 'sign' ? extra !== undefined && extra !== '--preflight' : extra !== undefined)))
    || (command === 'submit-recovery' && id !== 'phase3b-final-write-1')
    ...
    try { let result;
      ...
      else if (command === 'submit-recovery') result = extra === '--preflight' ? finalW1RecoveryPreflight() : await runFinalW1RecoverySubmit();
