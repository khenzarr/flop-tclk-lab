...
export function finalizeProductionJourney({ submitStateRoot = submitRoot, railStateRoot = railRoot, outputPath = output } = {}) {
  const observations = PUBLIC_ORDER.map(operationId => {
    const evidence = operationId === 'phase3b-write-5' || operationId === 'phase3b-write-6'
      ? read(resolve(railStateRoot, `${operationId}-observation.json`))
    ...
    if ((operationId === 'phase3b-write-5' || operationId === 'phase3b-write-6') && (evidence.evidenceClass !== 'UNSIGNED_RAIL_OBSERVATION' || evidence.valueMoved !== false)) throw new Error(`FINALIZE_PAPERRAIL_EVIDENCE_INVALID:${operationId}`);
    return evidence;
  });
